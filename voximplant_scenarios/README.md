...
- Кастомные функции (`functions`) вызываются в **исходящем** каскаде
  (`outbound_cascade`, tool-calling). Во входящем (`inbound_cascade`) —
  пока не вызываются.
