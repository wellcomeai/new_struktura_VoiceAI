...
`_rates` дополнительно отдаёт `credits_per_call` (оценка стоимости одного
вызова по эталону `REFERENCE_CALL_TOKENS = (4000, 500)`) и `tier`
(`low` / `mid` / `high` / `top` по границам `TIER_BOUNDS` — 2 / 10 / 40
кредитов за вызов). Фронт (`agent/core.js` → `modelOptionsHtml`,
`modelHintHtml`) рисует по ним цветной кружок и цену в селекте модели.
Пороги живут только на бэкенде и в JS не дублируются.
