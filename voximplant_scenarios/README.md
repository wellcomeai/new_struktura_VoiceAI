...
| `outbound_crm.js` | `outbound_crm` | **v5.6. Исходящий универсальный** сценарий (rule `outbound_crm`): 🟢 Gemini **2.5 / 3.1 / 3.8 Live**, 🔵 OpenAI Realtime, 🟠 Cartesia (OpenAI text + Cartesia TTS), 🟡 Yandex Realtime. Сюда идут исходящие агента обзвона, публичного API и CRM-задач для всех типов, кроме каскада и Fish (`task_scheduler._outbound_rule_name`). Точка входа — `AppEvents.Started` + `VoxEngine.customData()` (phone_number, assistant_id, assistant_type, caller_id, mute_duration_ms, contact_name/task_*/custom_greeting/task). Конфиг с `/api/telephony/outbound-config`. Ветка Gemini: WS к Gemini поднимается ДО `callPSTN` (retry ×1), приветствие через симулированный «Алло» + якорь в промпте, mute-окно 3с, VAD HIGH/HIGH, пинг тишины 8с (по `WebSocketMediaEnded`) + хард 180с, Session TTL 10 мин. Версия Gemini — по подстроке в `config.model`, проверка «3.8» раньше «3.1»: у 3.8 `thinkingConfig` не отправляется, runtime-текст через `sendClientContent`; у 3.1 `thinkingLevel:minimal` + `sendRealtimeInput`; у 2.5 `thinkingBudget:0`. Extended-thinking-вариант 3.8 не поддержан (turnComplete там не означает свободную сессию). Раскатка на дочерние: `/api/telephony/admin/deploy-scenarios`. |
