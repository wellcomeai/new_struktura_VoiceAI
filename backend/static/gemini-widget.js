/**
 * 🚀 Gemini Voice Widget v2.8.0 - PRODUCTION (GEMINI VAD)
 * Google Gemini Live API Integration
 * 
 * ✅ v2.8.0: PURE GEMINI VAD - removed client-side commit logic
 * ✅ Continuous audio streaming - Gemini decides when to respond
 * ✅ Client VAD only for UI visualization
 * ✅ Fixes "short phrase" problem (e.g., "Привет")
...
(function() {
    ...
    const CONFIG = {
        ...
        vad: {
            // ✅ ONLY for UI visualization - NOT for commit logic
            enabled: true,
            speechThreshold: -38,  // Detect when user is speaking (for UI only)
            visualSilenceThreshold: -45  // For audio bars animation
            // ❌ REMOVED: silenceDuration - Gemini handles this!
        },
...
`;
    ...
    function init() {
        console.log('[GEMINI-WIDGET] 🚀 Initializing v2.8.0 (PURE GEMINI VAD)...');
    ...
    // ============================================================================
    // AUDIO RECORDING - PURE GEMINI VAD
    // ============================================================================
    ...
    async function startRecording() {
        ...
        console.log('[GEMINI-WIDGET] 🎙️ Starting recording (pure Gemini VAD mode)...');
        ...
        try {
            ...
            console.log('[GEMINI-WIDGET] ✅ Recording started (continuous streaming)');
            console.log('[GEMINI-WIDGET] ℹ️ Gemini will decide when to respond');
    ...
    async function startAudioWorkletRecording(source) {
        ...
        workletNode.port.onmessage = (event) => {
            ...
            // ✅ Detect speech ONLY for UI state (not for commit!)
            const rms = calculateRMS(audioData);
            const db = 20 * Math.log10(rms);
            
            if (db > CONFIG.vad.speechThreshold) {
                if (!STATE.isSpeaking) {
                    console.log('[GEMINI-WIDGET] 🗣️ User speaking (UI only)');
                    sendMessage({ type: 'speech.user_started' });
                    STATE.isSpeaking = true;
                }
                STATE.lastSpeechTime = Date.now();
            } else if (STATE.isSpeaking && db < CONFIG.vad.visualSilenceThreshold) {
                // Only update state for UI - NO commit logic!
                if (Date.now() - STATE.lastSpeechTime > 500) {
                    console.log('[GEMINI-WIDGET] 🤐 User stopped (UI only)');
                    sendMessage({ type: 'speech.user_stopped' });
                    STATE.isSpeaking = false;
                }
            }
            
            // ✅ JUST SEND AUDIO - Gemini handles everything else
    ...
    async function stopRecording() {
        ...
        // ❌ NO MORE COMMIT - Gemini handles this automatically!
    ...
    console.log('[GEMINI-WIDGET] 🚀 Script loaded v2.8.0 (PURE GEMINI VAD)');
