...
class TestNumberLease(Base, BaseModel):
    ...
    # Аренда из обязательного онбординга (первый звонок после регистрации).
    # В лимит попыток не входит: после онбординга у пользователя остаётся
    # обычная базовая попытка плюс гранты админа.
    is_onboarding = Column(Boolean, default=False, nullable=False, server_default="false")
