...
селекте модели: `GPT-6 Luna (рекомендуем) · ~1.24 кр/звонок`, а под селектом —
...
«что выбрать» даёт флаг `is_recommended` у модели (сейчас — GPT-6 Luna, она же
`is_default`); это два независимых флага. Для неизвестного слага `calculate_cost`
берёт ставки `FALLBACK_RATES_SLUG` (DeepSeek V4 Pro), а не модели по умолчанию.
