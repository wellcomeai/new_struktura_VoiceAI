...
composio_user_id = user_id владельца голосового ассистента (то же подключение,
что использует оркестратор). Slug'и Composio фиксированы; аргументы передаются
напрямую под именами, которые ждёт Composio.
"""
