 ...
 * Версия: 2.0.2 - Исправлена поддержка iOS аудио
 * 
 * Исправления:
 * - Исправлена ошибка interruptionState is not defined в cleanupAudio
 * - Улучшена поддержка iOS аудио с playsInline
 * - Добавлены обработчики touchstart/click для разблокировки iOS
 * - Оптимизирована система воспроизведения аудио
 */
...
(function() {
  ...
  // Обновление отладочной панели (заглушка)
  ...
  async function testAudioPlayback() {
    return new Promise((resolve) => {
      try {
        ...
        testAudio.playsInline = true; // Критично для iOS
  ...
  async function initializeAudio() {
    ...
    try {
      ...
      if (isIOS) {
        // Принудительно разблокиваем аудио контекст для iOS
        document.addEventListener('touchstart', function unlockiOS() {
          if (window.globalAudioContext && window.globalAudioContext.state === 'suspended') {
            window.globalAudioContext.resume().then(() => {
              widgetLog('[AUDIO] AudioContext разблокирован через touchstart на iOS');
              window.audioPlaybackUnlocked = true;
            });
          }
        ...
        document.addEventListener('click', function unlockiOSClick() {
          if (!window.audioPlaybackUnlocked) {
            ...
            testAudio.play().then(() => {
              window.audioPlaybackUnlocked = true;
              widgetLog('[AUDIO] Воспроизведение разблокировано через click на iOS');
            }).catch(() => {
              widgetLog('[AUDIO] Не удалось разблокировать через click на iOS', 'warn');
            });
          }
          document.removeEventListener('click', unlockiOSClick);
        }, { once: true });
      }
      ...
      if (!window.globalAudioContext) {
        ...
        if (isIOS) {
          ...
          contextOptions.latencyHint = 'balanced';
        }
      ...
      if (!window.globalMicStream) {
        ...
        // Дополнительные ограничения для iOS
        if (isIOS) {
          constraints.audio.latency = 0.1; // Минимальная задержка для iOS
        }
      ...
      // 6. УЛУЧШЕННОЕ тестирование HTML5 Audio для iOS
      if (isMobile && !window.audioPlaybackUnlocked) {
        ...
        try {
          ...
          widgetLog(`[AUDIO] HTML5 Audio заблокировано: ${error.message}`, 'warn');
          // Не ошибка - просто потребуется взаимодействие пользователя
        }
      } else if (!isMobile) {
  ...
  function createCleanupAudio(interruptionState) {
    return function cleanupAudio(audioUrl, audio) {
      ...
      if (audio && interruptionState && interruptionState.current_audio_elements) {
            ...
            playPromise
              ...
              .catch(error => {
                ...
                if (isMobile && (error.name === 'NotAllowedError' || error.name === 'AbortError')) {
                  ...
                  showAudioUnlockButton(audioUrl, audio, cleanupAudio, playNextAudio);
                } else {
  ...
  function initWidget() {
    ...
    // Глобальная переменная для isPlayingAudio
    window.isPlayingAudio = false;
    ...
    function switchToListeningMode() {
      ...
      if (isConnected && !isReconnecting) {
        setTimeout(() => {
          if (!isListening && !window.isPlayingAudio) {
            startListening();
          }
        }, 100);
      }
    ...
    function handleAssistantSpeechEnded(eventData) {
      ...
      // ВАЖНО: НЕ отключаем микрофон, он должен быть постоянно активен как в десктопе
      // Автоматически начинаем слушать как в десктопной версии
    ...
    async function openWidget() {
      ...
      if (isConnected && !isListening && !window.isPlayingAudio && !isReconnecting) {
        ...
        widgetLog(`Cannot start listening yet: isConnected=${isConnected}, isListening=${isListening}, isPlayingAudio=${window.isPlayingAudio}, isReconnecting=${isReconnecting}`);
    ...
    // Начало записи голоса - УНИФИЦИРОВАННОЕ для всех устройств
    async function startListening() {
      ...
      if (!audioProcessor) {
        const bufferSize = 2048; // Единый размер буфера для всех устройств
          ...
          connectWebSocket().then(success => {
            if (success) {
              ...
              if (isWidgetOpen) {
                ...
                setTimeout(() => {
                  if (isWidgetOpen && !isListening && !window.isPlayingAudio) {
                    startListening();
                  }
                }, 1000);
              }
            ...
            try {
              ...
              if (data.type === 'error') {
                if (data.error && data.error.code === 'input_audio_buffer_commit_empty') {
                  ...
                  if (isWidgetOpen && !window.isPlayingAudio && !isReconnecting) {
                    setTimeout(() => { 
                      startListening(); 
                    }, 500);
                  }
    ...
    mainCircle.addEventListener('click', function() {
      widgetLog(`Circle clicked: isWidgetOpen=${isWidgetOpen}, isListening=${isListening}, isPlayingAudio=${window.isPlayingAudio}, isReconnecting=${isReconnecting}`);
      
      if (isWidgetOpen && !isListening && !window.isPlayingAudio && !isReconnecting) {
        if (isConnected) {
          startListening();
  ...
  function initializeWidget() {
    widgetLog('Initializing unified widget...');
