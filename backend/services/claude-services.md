...
- `agent_memory.py` — **память самого агента** (блокнот оркестратора, `agent_configs.memory` JSONB): заметки с id по секциям `instructions` / `observations` / `plans`. Правки только точечные — `apply_ops(add/update/delete)` (чистая функция) и `lock_and_apply` (под `SELECT … FOR UPDATE` строки агента, чтобы параллельные PostCall разных контактов не затирали друг друга). `render_block` собирает блок «ПАМЯТЬ АГЕНТА», который `agent_prompts.build_agent_memory_block` приклеивает к user-сообщению во всех фазах v3 (PreCall, PostCall, чат, Telegram-чат, публичный канал). Лимиты `MAX_NOTES=60`, `MAX_NOTE_CHARS=400`, `MAX_TOTAL_CHARS=8000`. Тулза агента — `update_agent_memory` (в `AGENT_CHAT_TOOLS` и `AGENT_POSTCALL_TOOLS`); владелец правит через `/api/agent/memory`.
