...
| `max-account.js` | ЛИЧНЫЙ аккаунт мессенджера MAX агента (PyMax): модалка (телефон → SMS-код → пароль 2FA). Отличие от Telegram: авторизация PyMax асинхронная, поэтому после отправки кода/пароля UI ОПРАШИВАЕТ статус (`_maxAccPollStatus`), пока не станет connected/pending_password/error. Строка живёт в списке коннекторов — `connectors.js` зовёт `maxAccountConnectorRowHtml`/`maxAccountSummaryHtml`/`maxAccountAvailable` | `loadMaxAccount`, `openMaxAccountModal`, `renderMaxAccountModal`, `maxAccStart/VerifyCode/VerifyPassword/Restart`, `maxAccToggleAutoReply`, `maxAccSetScope`, `maxAccDisconnect`; состояние `maxAccState`, константа `MAX_ACC_API` | `/api/agent/max-account/*` |
| `contacts.js` | CRM-контакты | `saveContact`, `openContactsListModal`, `loadContactsList` (первые `CONTACTS_PAGE`=100, дальше кнопка «Показать ещё» → `loadMoreContacts`, состояние `contactsListState`), `exportContacts` (скачивание всей базы агента в xlsx, кнопки с классом `.contacts-export-btn` блокируются на время загрузки), `openContactDetailsModal`, `renderContactDetails`, `cdTask*`, `_cdSmsBubble` (SMS-переписка в карточке), `saveContactInfo`, `changeContactStage` | `/api/agent/contacts*`, `GET /api/agent/contacts/export` |
...
| `import.js` | Импорт контактов (xlsx/csv, 3 шага). Ползунок авто-задач: включён — стоимость по всем контактам, при нехватке кредитов импорт заблокирован; снят — контакты загружаются целиком вне зависимости от баланса (`can_proceed_without_tasks`), стоимость задач из файла показывается справочно | `openImportModal`, `handleImportFile`, `renderImportPreview`, `renderImportCostLine`, `onImportTasksToggle` (ползунок авто-задач), `updateImportProceedBtn`, `executeImport`, `pollImportStatus` (реальный прогресс-бар, опрос раз в секунду), `finishImport`; состояние `importState`. До 10 000 строк, превью отдаёт `errors_count`/`duplicates_count` и только начало списков | `/api/agent/contacts/import/*` (execute: `create_tasks`; без него проверки баланса нет; `status/{token}` — прогресс) |
| `pipeline.js` | Воронка-канбан (drag&drop), ленивая загрузка: каждая стадия грузится своим запросом (`PIPELINE_PAGE`=100), в заголовке колонки реальный total, внизу кнопка «Ещё N» → `plLoadMore`; состояние `plState` | `openPipelineModal`, `loadPipeline`, `fetchPipelineStage`, `plLoadMore`, `pipelineCard`, `plDragStart/End/Over/Leave`, `plDrop` | `GET /api/agent/contacts?status=&limit=&offset=`, `PATCH /contacts/{id}/status` |
