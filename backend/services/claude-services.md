...
- **Оркестратор:** `PreCallOrchestrator.run(...)`, `PostCallOrchestrator.poll_and_run(...)`, `ChatOrchestrator.run(...)` / `.run_telegram(...)`. Все три выбирают ветку v3 (OpenRouter, `_run_v3_openrouter` / `_analyze_v3_openrouter` / `_run_telegram_v3`) или v2 (OpenAI Responses API, `store=True`). Usage токенов достаётся через `_extract_usage`.
- **Tools агента:** `execute_tool(tool_name, tool_args, context, db)` — единая точка диспетчеризации; `context` обязан содержать `agent_config_id`, `user_id`, `user`. PostCall ВСЕГДА создаёт задачу на перезвон, кроме случая достижения цели; `fn_create_agent_task` отменяет дубли SCHEDULED-задач этого контакта.
