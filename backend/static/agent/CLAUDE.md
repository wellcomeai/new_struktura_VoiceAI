...
| `calls.js` | История звонков (модалка) | `openCallsModal`, `renderCallExpanded` (SMS-aware: `call.channel`), `toggleCallCard`; `POSTCALL_TOOL_LABELS` (все 7 тулов PostCall) | `/api/agent/calls`, `/calls/{id}` |
