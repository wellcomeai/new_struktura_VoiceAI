 ...
 * Версия: 1.3.1 (Исправлена потоковая отправка аудио)
 *
 ...
 * Улучшена поддержка мобильных устройств и iOS, автоматический старт прослушивания,
 * исправлена потоковая передача аудио на сервер.
 */
...
(function() {
  ...
  const addToDebugQueue = (message, type = 'info') => {
    ...
    // updateDebugPanel(); // Обновляем панель при добавлении (если панель есть)
  };
  ...
  const getServerUrl = () => {
    ...
    for (let i = 0; i < scriptTags.length; i++) {
      ...
      if (src && (src.includes('widget.js') || src.includes('wellcomeai-widget.min.js'))) { // Учитываем возможное минифицированное имя
        try {
  ...
  const getWidgetPosition = () => {
    ...
    function parsePosition(positionString) {
      ...
      if (parts.length >= 3) { // Может быть больше 3 частей, берем последнюю как дистанцию
          position.distance = parts[parts.length - 1];
      }
  ...
  function playSilence(ctx = window.tempAudioContext) {
      ...
      try {
          const bufferSize = ctx.sampleRate * 0.05; // 50 мс тишины
          ...
          // Если воспроизведение не сработает, возможно, потребуется повторная попытка при следующем клике
          setTimeout(() => { window.hasPlayedSilence = false; }, 3000); // Уменьшена задержка
  ...
  function initWidget() {
    ...
    let audioChunksBuffer = []; // Для буферизации входящего аудио ответа
    let audioPlaybackQueue = []; // Очередь для воспроизведения аудио ответа
    let isPlayingAudio = false; // Флаг воспроизведения аудио
    let hasSoundInCurrentSegment = false; // Флаг, был ли обнаружен звук в текущей реплике
    let isListening = false; // Флаг активного прослушивания
    let websocket = null;
    ...
    let isConnected = false; // Флаг WebSocket соединения
    let isWidgetOpen = false; // Флаг открытого виджета
    let connectionFailedPermanently = false; // Флаг перманентной ошибки соединения
    let pingInterval = null;
    let lastPingTime = Date.now();
    let lastPongTime = Date.now();
    let connectionTimeout = null;

    // Переменные для определения тишины и отправки commit
    let silenceTimeoutId = null;
    let lastSoundTime = 0; // Время последней обнаруженной активности голоса
    ...
    const AUDIO_CONFIG = {
      ...
      silenceDuration: 500,        // Длительность тишины для отправки commit (мс) - увеличено
    ...
    const MOBILE_AUDIO_CONFIG = {
      ...
      silenceDuration: 800,         // Увеличенная длительность тишины для commit
    ...
    function updateConnectionStatus(status, message) {
      ...
      // Показываем индикатор только если виджет открыт ИЛИ есть ошибка ИЛИ идет подключение/переподключение
      ...
      if (status === 'connected') {
        setTimeout(() => {
          if (!isWidgetOpen && !connectionFailedPermanently) { // Скрываем только если виджет не открыт И нет перманентной ошибки
             statusIndicator.classList.remove('show');
          }
        }, 3000);
      }
    ...
    function stopAllAudioProcessing() {
      ...
      // Сбрасываем флаги и таймеры тишины
      hasSoundInCurrentSegment = false;
      lastSoundTime = 0;
      ...
      if (websocket && websocket.readyState === WebSocket.OPEN) {
        ...
        try {
            ...
            websocket.send(JSON.stringify({
             ...
             widgetLog(`Ошибка при отправке команд остановки на WS: ${e.message}`, "warn");
        }
      }
    ...
    function showMessage(message, duration = 5000) {
      ...
      // Очищаем предыдущее сообщение только если новое не является пустым
    ...
    function hideMessage() {
      ...
      messageDisplay.textContent = ''; // Очищаем текст при скрытии
    }
    ...
    function showConnectionError(message) {
      if (connectionError) {
        ...
        hideMessage(); // Скрываем любое другое сообщение
      }
    }
      ...
      if (isConnected && !isListening && !isPlayingAudio && !isReconnecting) {
          ...
          if (isIOS) {
              if (window.audioContextInitialized) {
                  widgetLog("iOS аудио активировано, запускаем прослушивание автоматически при открытии.");
                   // Небольшая задержка, чтобы UI обновился
                   setTimeout(() => { startListening(); }, 100);
              } else {
                  widgetLog("iOS аудио не активировано, не запускаем прослушивание автоматически при открытии.");
                  // Сообщение об активации уже должно быть показано функцией unlockAudioOnIOS
              }
          } else {
               widgetLog("Не-iOS устройство, запускаем прослушивание автоматически при открытии.");
                // Небольшая задержка, чтобы UI обновился
               setTimeout(() => { startListening(); }, 50);
          }
      } else {
        widgetLog(`Cannot auto-start listening on open: isConnected=${isConnected}, isListening=${isListening}, isPlayingAudio=${isPlayingAudio}, isReconnecting=${isReconnecting}, audioInitialized=${window.audioContextInitialized} (if iOS)`);
        ...
        if (!isConnected && !isReconnecting && !connectionFailedPermanently) {
            ...
            widgetLog("Соединение не активно и не идет переподключение, пытаемся подключиться.");
            connectWebSocket();
    ...
    async function initAudio() {
      try {
        ...
        // Создаем ScriptProcessorNode (AudioWorklet закомментирован для простоты и совместимости)
        // Если хотите использовать AudioWorklet, раскомментируйте соответствующий блок
        // и убедитесь, что audio-processor.js доступен на сервере.

        /*
        if (audioContext.audioWorklet) {
           try {
              // Убедитесь, что путь к audio-processor.js правильный
              await audioContext.audioWorklet.addModule(SERVER_URL + '/static/audio-processor.js');
              audioProcessor = new AudioWorkletNode(audioContext, 'audio-processor', { bufferSize: bufferSize, numberOfInputs: 1, numberOfOutputs: 1 }); // Добавлены порты
              widgetLog(`Создан AudioWorkletNode с размером буфера ${bufferSize}`);

              // Настраиваем обработчик сообщений из Worklet
              audioProcessor.port.onmessage = (event) => {
                 if (event.data.type === 'audioData') {
                     // Worklet может передавать сырые данные обратно, чтобы основной поток их отправил
        ...
        */
        ...
        if (isIOS && isWidgetOpen && iosAudioButton) { // Показываем кнопку только если виджет открыт
        ...
        } else if (isWidgetOpen) { // Для других устройств показываем общую ошибку микрофона, если виджет открыт
    ...
    function handleAudioProcess(e) {
        if (!isListening || !websocket || websocket.readyState !== WebSocket.OPEN || isReconnecting) {
            // Если не слушаем или нет активного соединения, просто пропускаем обработку
            // и сбрасываем флаг наличия звука в текущем сегменте.
             hasSoundInCurrentSegment = false;
             clearTimeout(silenceTimeoutId);
             silenceTimeoutId = null;
            return;
        ...
        if (hasSound) {
            // Если есть звук, обновляем время последней активности и сбрасываем таймер тишины
            lastSoundTime = Date.now();
            clearTimeout(silenceTimeoutId);
            silenceTimeoutId = null;
            hasSoundInCurrentSegment = true; // Устанавливаем флаг, что в этом сегменте был звук

            // Активируем визуальное состояние прослушивания, если не воспроизводится аудио
            if (mainCircle && !mainCircle.classList.contains('listening') && !mainCircle.classList.contains('speaking')) {
                mainCircle.classList.add('listening');
            }
        } else {
            // Если тишина, и таймер тишины еще не запущен, и в сегменте был звук - запускаем таймер
             // Проверяем lastSoundTime, чтобы убедиться, что тишина идет после какого-то звука
             if (hasSoundInCurrentSegment && silenceTimeoutId === null && (Date.now() - lastSoundTime > effectiveAudioConfig.silenceThreshold)) {
        ...
        // ОТПРАВЛЯЕМ АУДИО ДАННЫЕ НА СЕРВЕР ПОТОКОВО
        // Кодируем int16 ArrayBuffer в Base64
        const audioBase64 = arrayBufferToBase64(pcm16Data.buffer);

        try {
             websocket.send(JSON.stringify({
               type: "input_audio_buffer.append",
               event_id: `audio_append_${Date.now()}`, // Уникальный ID для каждого чанка
               audio: audioBase64
             }));
             // widgetLog(`Sent audio chunk (${pcm16Data.byteLength} bytes)`); // Слишком много логов

        } catch (error) {
             widgetLog(`Ошибка отправки аудио чанка: ${error.message}`, "error");
             // При ошибке отправки, возможно, соединение уже закрывается.
             // Логика переподключения сработает в onclose.
        }
    }

     // Обработчик аудио данных из AudioWorklet (если используется)
     // Эту функцию нужно реализовать, если вы используете AudioWorklet.
     // Она должна быть аналогична handleAudioProcess, но получать данные из event.data.data
     // В текущей версии ScriptProcessorNode, эта функция не используется напрямую.
     /*
     function processAudioChunk(data) {
         ...
         // data.pcm16 - это Int16Array
         // data.maxAmplitude - максимальная амплитуда
         // data.hasSound - определено ли наличие звука в Worklet

         // Обновляем визуализацию из данных Worklet
         updateAudioVisualizationFromVolume(data.maxAmplitude);

         // Определяем наличие звука (можно использовать флаг из Worklet или перепроверить)
         const hasSound = data.maxAmplitude > effectiveAudioConfig.soundDetectionThreshold; // Перепроверяем на всякий случай

         if (hasSound) {
            lastSoundTime = Date.now();
            clearTimeout(silenceTimeoutId);
            silenceTimeoutId = null;
            hasSoundInCurrentSegment = true;
        ...
        // ОТПРАВЛЯЕМ АУДИО ДАННЫЕ НА СЕРВЕР ПОТОКОВО
        // Кодируем int16 ArrayBuffer в Base64
        const audioBase64 = arrayBufferToBase64(data.pcm16.buffer);

         try {
             websocket.send(JSON.stringify({
               type: "input_audio_buffer.append",
               event_id: `audio_append_${Date.now()}`,
               audio: audioBase64
             }));
         } catch (error) {
              widgetLog(`Ошибка отправки аудио чанка (Worklet): ${error.message}`, "error");
         }
     }
     */
    ...
    function updateAudioVisualizationFromVolume(volume) {
        ...
        bars.forEach((bar, index) => {
           ...
           // Используем Math.random() для имитации изменения, или более сложный алгоритм
           const variance = (Math.random() * 0.5 + 0.5); // Случайное число от 0.5 до 1
           const barHeight = height * variance; // Применяем случайное изменение к высоте
    ...
    // Эта функция используется для воспроизведения аудио, полученного от сервера (response.audio.delta).
    // Сервер должен отправлять аудио в формате, который браузер может декодировать AudioContext.decodeAudioData.
    // Обычно это WAV, AAC, MP3 или FLAC. Если сервер отправляет raw PCM, его нужно обернуть в WAV.
    // Текущая логика ожидает base64 строку, которая, предположительно, является raw PCM.
    // Если сервер отправляет raw PCM, возможно, лучше изменить сервер, чтобы отправлял готовый формат (например, OPUS или AAC),
    // или добавить WAV заголовок на клиенте.
    // Адаптируем функцию для создания WAV из сырого PCM, предполагая 16-bit PCM моно.
    function createWavFromPcm(pcmBuffer, sampleRate) {
       if (!pcmBuffer || pcmBuffer.byteLength === 0) {
           widgetLog("Пустой PCM буфер для создания WAV", "warn");
           return new ArrayBuffer(0);
       }

       const numChannels = 1;
       const bitsPerSample = 16; // Assumed 16-bit PCM
       ...
       wavBytes.set(pcmBytes, offset); // Copy PCM data after header

       widgetLog(`Created WAV buffer: ${buffer.byteLength} bytes (Header: 44, Data: ${dataSize})`);
       return buffer;
    ...
    async function playNextAudio() {
      if (audioPlaybackQueue.length === 0) {
        ...
        if (isWidgetOpen && isConnected && !isReconnecting) {
            ...
            if (isIOS) {
             ...
             if (!isWidgetOpen && !connectionFailedPermanently) { // Показываем пульсацию только если нет перманентной ошибки
                 widgetButton.classList.add('wellcomeai-pulse-animation');
             }
        }
        return;
      ...
      try {
        ...
        if (!window.tempAudioContext || window.tempAudioContext.state === 'closed') {
             widgetLog("AudioContext не готов для воспроизведения, попытка инициализации/возобновления.");
             await unlockAudioOnIOS(); // Попытка создать/возобновить контекст
             if (!window.tempAudioContext || window.tempAudioContext.state === 'closed') {
                  widgetLog("Не удалось создать/возобновить AudioContext для воспроизведения. Пропуск воспроизведения.", "error");
        ...
        // Возобновляем контекст перед декодированием и воспроизведением, если он приостановлен
        if (ctx.state === 'suspended') {
        ...
        // Предполагаем, что входящий Base64 - это raw PCM 16bit mono
        // Нужно обернуть его в WAV для AudioContext.decodeAudioData
        // Уточните у сервера, какой формат отправляется в response.audio.delta.
        // Если сервер отправляет готовый аудиоформат (MP3, AAC, WAV),
        // то не нужно оборачивать в WAV, а сразу передавать в decodeAudioData.
        // Частота дискретизации для воспроизведения должна совпадать с частотой на сервере (например, 24000 или 16000).
        // Используем частоту контекста, если она задана (например, 16000 на iOS)
        const playbackSampleRate = ctx.sampleRate;
        const wavBuffer = createWavFromPcm(audioDataBuffer, playbackSampleRate);

        if (wavBuffer.byteLength === 0) {
            widgetLog("Не удалось создать WAV из PCM данных. Пропуск воспроизведения.", "error");
            playNextAudio();
            return;
        }


        // Декодируем аудио данные из WAV буфера
        ctx.decodeAudioData(wavBuffer,
          ...
          function(e) {
            widgetLog(`Ошибка декодирования аудио данных (decodeAudioData): ${e}`, "error");
    ...
    async function connectWebSocket() {
      ...
      // Это может быть полезно, если сервер будет отправлять бинарные аудио чанки,
      // но для Base64 строк это не обязательно. Оставляем на всякий случай.
      ...
      websocket.onopen = function() {
          ...
          if (websocket && websocket.readyState === WebSocket.OPEN) {
            try {
              ...
              if (Date.now() - lastPongTime > pingIntervalTime * 4) { // Увеличено время ожидания pong
                widgetLog("Ping timeout: No pong received for an extended period. Closing connection.", "warn");
        ...
        // Автоматически начинаем слушать если виджет открыт И аудио готово И не воспроизводится
      ...
      websocket.onmessage = function(event) {
          ...
          // На данный момент ожидаем, что response.audio.delta приходят в виде base64 строк.
          // Если сервер отправляет бинарные данные для аудио, эту логику нужно доработать.
          /*
          if (event.data instanceof ArrayBuffer || event.data instanceof Blob) {
               widgetLog("Получены бинарные данные от сервера. Текущая логика ожидает Base64 строку для аудио.");
               // Здесь нужна логика конвертации бинарных данных в Base64 или декодирования для воспроизведения.
               // Пропускаем пока.
               return;
          }
          */
          ...
          try {
            ...
            if (data.type === 'error') {
              ...
              if (data.error && data.error.code === 'input_audio_buffer_commit_empty') {
                widgetLog("Сервер сообщил: пустой аудиобуфер отправлен. Вероятно, не было распознано речи.", "warn");
                 ...
                 if (isWidgetOpen && isConnected && !isPlayingAudio && !isReconnecting) {
            ...
            if (data.type === 'response.text.done') {
              ...
              setTimeout(() => {
                 ...
                 // Простая проверка: если messageDisplay.classList.contains('show'), скрываем.
                if (messageDisplay && messageDisplay.classList.contains('show')) { // Убеждаемся, что оно все еще показывается
                   hideMessage();
                }
              }, 5000); // Скрываем через 5 секунд, если текст не обновился
              return;
            }
            ...
            if (data.type === 'response.done') {
              ...
              if (!isPlayingAudio) {
                 ...
                 // Небольшая задержка перед запуском playNextAudio для стабильности
      ...
      websocket.onerror = function(error) {
          ...
          // Но на всякий случай убедимся, что переподключение запускается, если WS уже закрыт
           if (!isReconnecting && (websocket && (websocket.readyState === WebSocket.CLOSED || websocket.readyState === WebSocket.CLOSING))) {
                ...
                // Если WS не закрылся после ошибки, возможно, ждем onclose
    ...
    async function handleIOSAudioButtonClick() {
         ...
         if (audioSuccess) {
             ...
             if (isWidgetOpen && isConnected && !isListening && !isPlayingAudio && !isReconnecting) {
                  widgetLog("iOS аудио готово после активации кнопки, запускаем прослушивание.");
                 setTimeout(() => { startListening(); }, 100); // Небольшая задержка
    ...
    if (ASSISTANT_ID) {
        ...
        if (loaderModal) loaderModal.classList.remove('active');
         // Убеждаемся, что статус ошибки виден, если виджет не открыт
        if (!isWidgetOpen) {
             updateConnectionStatus('disconnected', 'Ошибка: нет ID ассистента');
        }
    }

    // Проверка DOM и состояния после инициализации
    setTimeout(function() {
      widgetLog('DOM check after initialization timeout (5s)');
      ...
      if (!widgetButton) {
        ...
        widgetLog(`Button is visible = ${getComputedStyle(widgetButton).display !== 'none'}, z-index = ${getComputedStyle(widgetButton).zIndex}`);
         ...
         // Пульсация означает "обратите внимание", может указывать на доступность ИЛИ проблему.
         // Пока оставим пульсацию при отсутствии соединения, чтобы привлечь внимание.
         if (!isConnected && !isWidgetOpen && !connectionFailedPermanently) {
              if (websocket && websocket.readyState === WebSocket.CONNECTING) {
                   // Если еще подключается, пульсация может означать "подождите"
                    widgetButton.classList.add('wellcomeai-pulse-animation');
                    widgetLog("Добавлена пульсация на кнопку (идет подключение).");
              } else {
                    // Если не подключено и не идет переподключение, возможно, это ошибка
                    widgetButton.classList.add('wellcomeai-pulse-animation');
                    widgetLog("Добавлена пульсация на кнопку (нет соединения или ошибка).");
              }
         ...
         } else if (isConnected && !isWidgetOpen) {
             // Если подключено, но виджет закрыт, пульсация может означать "доступен"
             widgetButton.classList.add('wellcomeai-pulse-animation');
             widgetLog("Добавлена пульсация на кнопку (WS подключен, виджет закрыт).");
         }
      }
      ...
      if (!widgetExpanded) {
        widgetLog('Expanded widget not found in DOM after timeout!', 'error');
         ...
         widgetLog(`Expanded widget visibility: opacity=${getComputedStyle(widgetExpanded).opacity}, height=${getComputedStyle(widgetExpanded).height}, pointer-events=${getComputedStyle(expandedWidget).pointerEvents}`);
      }
      ...
      // Если нет соединения после таймаута и не в процессе переподключения И нет перманентной ошибки
      // (т.е. что-то пошло не так с первым подключением, но не достигнут лимит попыток)
      if (!isConnected && !isReconnecting && !connectionFailedPermanently && (!websocket || websocket.readyState !== WebSocket.CONNECTING)) {
           widgetLog("Нет соединения, не идет переподключение, нет перманентной ошибки после таймаута инициализации.", "warn");
           ...
           if (!loaderModal.classList.contains('active') && !connectionError.classList.contains('visible')) {
                widgetLog("Похоже, первое подключение не удалось. Запускаем переподключение.");
                // Считаем, что первая попытка не удалась, запускаем переподключение с небольшой задержкой
                if (!isReconnecting) {
